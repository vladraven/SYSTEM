...
series_mapping_stale: HTTP 404, таблица/вектор удалены, API deprecation. Production use блокируется. Возврат в valid только после успешной новой автоматической проверки (запущенной через CLI или CI).
...
Нативный float обнаружен: FLOAT_USAGE_FORBIDDEN (HTTP 500/422). Блокируется статическим анализом и/или runtime-проверкой в Decimal Value Object.
...
POST /api/v1/calculations/production: Роли: Admin, Risk Officer. Payload: config_version, vintage_date. Возвращает RiskScoreResult. Если bootstrap не пройден -> HTTP 422.
...
СИСТЕМНЫЕ И КАЛИБРОВОЧНЫЕ КОНСТАНТЫ (SYSTEM CONSTANTS)
...
TIMEZONE: UTC
SEED_ENDIANNESS: BIG_ENDIAN (Unsigned 64-bit integer interpretation)
SCALE: 8 (Внутренняя точность вычислений BCMath)
STORAGE_SCALE: 4 (Точность сохранения в БД DECIMAL 10,4. Округление: Half Up).
NORMALIZATION_EPSILON: 0.00000001 (Guard-порог защиты от деления на ноль)
...
(Эти параметры выведены из калибровки на исторических данных Манитобы и не являются доказанными универсальными законами природы).
GAMMA_SCALE: 0.9543 (Компенсирует дискретность годового старения когорты)
CALIBRATION_G9: 1.0000 (Residual internal retention effect, default)
CALIBRATION_G10: 1.0000 (Migration replenishment scale, default)
...
МАТЕМАТИЧЕСКАЯ ТОЧНОСТЬ (DECIMAL POLICY)
...
Любое использование функций floatval(), (float) при расчёте risk_score строго запрещено. Выбрасывается исключение FLOAT_USAGE_FORBIDDEN (через статический анализ или runtime-checks).
ТЕРМИНОЛОГИЯ: ELIGIBLE INDICATORS
...
Effective weights (Расчёт эффективных весов)
...
СТРАТЕГИИ НОРМАЛИЗАЦИИ (THRESHOLD NORMALIZATION)
...
ЭФФЕКТИВНЫЕ ВЕСА И ROUNDING RECONCILIATION
...
Если delta != 0.0000, delta детерминированно прибавляется к ОДНОМУ индикатору. (В крайнем случае, когда все w_disc стремились к нулю и округлились в 0, один индикатор получит ~100.0000).
...
ИТОГОВЫЕ РАСЧЕТЫ RISK SCORE И КАТЕГОРИЙ
...
Примечание: Сумма category_contribution не обязана строго равняться общему risk_score из-за frequency_discounts и перераспределения весов между категориями.
КЛАССИФИКАЦИЯ RISK BANDS
Инвариант: разбиение отрезака [0.0000, 100.0000] должно быть непрерывным, без дыр и перекрытий.
...
ОПЦИОНАЛЬНОЕ РАСШИРЕНИЕ: ДЕМОГРАФИЧЕСКИЙ МЕТАБОЛИЗМ (ACMF)
