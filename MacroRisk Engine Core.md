...
Язык: PHP 8.3+ (Framework-independent, strict type-checking, native BCMath)

СУБД: MySQL 8.4 LTS InnoDB utf8mb4 (utf8mb4_0900_as_cs для отображения; для indicator_key и всех полей, участвующих в детерминированной сортировке для построения хешей, значения ограничены ASCII — см. Файл 3, indicator_configs — сортировка в коде выполняется явным ASCII byte-wise сравнением независимо от коллации СУБД, коллация СУБД используется только для UI/отображения)

Локаль: en-CA (см. §6.1 — суженный формат ^[a-z]{2}-[A-Z]{2}$, не полный BCP-47; fr-CA в Phase 3)
...
Значение, вычисленное get_effective_release_date, при создании/обновлении snapshot_observations сохраняется в NOT NULL колонке snapshot_observations.release_date (вместе с соответствующим release_date_quality). Если обработка строки происходит до завершения классификации релиза, поле заполняется результатом вычисления ветки fallback_to_ingestion (ingestion_runs.started_at) с качеством 'fallback_to_ingestion' — наличие NULL в snapshot_observations.release_date категорически запрещено. Именно это поле, а не поле release_calendars напрямую, используется в правиле выбора ревизий §4, поскольку look-ahead bias оценивается на уровне доступности конкретного наблюдения (snapshot_observations), а не абстрактного релизного периода.
...
Обратный сброс release_late -> valid: При поступлении данных за задержанный период (data arrived), release_calendars.release_status меняется на 'revised' или закрывается фактом заполнения actual_release_date. В этот момент фоновый процесс синхронизации проверяет состояние последнего ожидавшегося периода серии: если он больше не является 'release_late' и отсутствуют прочие блокировки из source validation state machine, series.validation_status автоматически восстанавливается в 'valid'.
...
Область проекции в series.validation_status ограничена: из состояний release_calendars в series.validation_status синхронизируется переход в release_late (и его обратный сброс в valid). Остальные значения series.validation_status (temporary_unavailable, unavailable, access_denied, schema_mismatch, data_pending, series_mapping_stale, pending_validation, valid) возникают из независимого source validation state machine (§3.4) и не являются проекцией release_calendars — release_calendars не источник истины для всего validation_status, а источник истины только для состояния задержки релизов.
...
Из кандидатов snapshot_observations данной серии выбрать те, где snapshot_observations.release_date <= vintage_date (гарантировано NOT NULL, см. §3.1).
...
Канонический справочник типов отчётов (report_type):

situation_room_summary — оперативка Ситуационной комнаты (главный панельный отчёт).

backtest_executive_audit — сводный отчёт аудита бэктеста.

indicator_deep_dive — детальный разбор отдельной категории/индикатора.

stress_test_briefing — брифинг по результатам стресс-тестирования.

Для каждого сгенерированного отчёта (narrative_reports):

report_key формируется детерминированно ДО генерации текста:

report_key = HEX(SHA-256(risk_score_result_id || '|' || configuration_version_id || '|' || locale || '|' || report_type))

Идемпотентность: повторный запрос генерации отчёта с идентичными входами обязан вернуть уже существующую запись narrative_reports с этим report_key (по UNIQUE), а не выбросить ошибку дубликата и не создать новую запись.
...
Выбор варианта слота, для каждого требуемого slot_key:

a. Список одобренных вариантов — narrative_slot_translations, отфильтрованных по locale, где родительский narrative_slots.status = 'published' И narrative_slots.scientific_integrity_status = 'approved' (у самой narrative_slot_translations колонки status/scientific_integrity_status НЕТ — фильтрация всегда через родительскую narrative_slots), отсортированных по id ASC. N — их количество.

b. N = 0 -> SCIENTIFIC_INTEGRITY_VIOLATION.

c. slot_seed = первые 8 байт SHA-256(seed_hash || '|' || slot_key), BIG_ENDIAN unsigned 64-bit integer.

d. selected_index = slot_seed MOD N (0-based).

e. Фиксируется вариант с этим индексом; запись — в narrative_report_slots.
...
MINIMUM_COVERAGE_REQUIRED: 60.0000 (единственный источник истины и Hard Floor для покрытия; risk_configuration_versions.coverage_minimum НЕ имеет собственного DEFAULT в схеме и обязано устанавливаться приложением из этой константы явно на момент создания версии — см. Файл 3; ограничение также подкреплено CHECK-constraint'ом chk_coverage_min в DDL).
...
available: наблюдение существует на дату винтажа (effective_release_date <= vintage_date, где effective_release_date берётся из NOT NULL колонки snapshot_observations.release_date — см. Файл 1, §3.1), значение NOT NULL.
...
higher_is_riskier (H > L): x <= L -> 0.0000; x >= H -> 100.0000; иначе ((x-L)/(H-L))*100.0000.
lower_is_riskier (H < L): x >= L -> 0.0000; x <= H -> 100.0000; иначе ((L-x)/(L-H))*100.0000.
...
risk_score = СУММА(contribution_i) для всех i из Eligible.
...
Category Score
Для категории C — множество A_c (eligible indicators в категории). Если A_c пусто: category_score = NULL, category_contribution = 0.0000.
cat_weight_i = (reconciled_effective_weight_i / СУММА(reconciled_effective_weight в A_c)) * 100.0000
category_score = СУММА(normalized_score_i * cat_weight_i) / 100.0000
category_contribution = category_score * СУММА(original_weight_i, i в A_c) / 100.0000
Сумма category_contribution не обязана строго равняться risk_score (интерпретационная декомпозиция).
...
СУБД: MySQL 8.4 LTS, Engine: InnoDB, Charset: utf8mb4 (utf8mb4_0900_as_cs)
...
-- Ограничение chk_coverage_min на уровне 60.0000 несёт комментарий-маркер со ссылкой
-- на константу MINIMUM_COVERAGE_REQUIRED.
CREATE TABLE risk_configuration_versions (
    ...
    CONSTRAINT chk_coverage_min CHECK (coverage_minimum >= 60.0000) COMMENT 'MUST equal MINIMUM_COVERAGE_REQUIRED from File 2 §1.2',
...
-- Значения detection_window_before_days и detection_window_after_days по умолчанию (90)
-- не несут DEFAULT в схеме: приложение явно передаёт значения из констант
-- DEFAULT_DETECTION_WINDOW_BEFORE_DAYS и DEFAULT_DETECTION_WINDOW_AFTER_DAYS из Файла 2, §1.2.
CREATE TABLE historical_episodes (
    ...
    detection_window_before_days INT UNSIGNED NOT NULL,
    detection_window_after_days INT UNSIGNED NOT NULL,
...
Главный принцип: система не должна притворяться, что знает то, чего не поддерживают данные. MacroRisk — система детерминированной диагностики структурной устойчивости, а не механизм прогнозирования. Риск-скор — исключительно модельная оценка, а не эмпирически наблюдаемым факт экономики.
