...
1. Normalization (Нормализация индикаторов).
2. Coverage (Проверка покрытия).
3. Effective weights (Расчёт эффективных весов с применением дисконта).
4. Rounding reconciliation (Округление и примирение).
5. Indicator contribution (Вклад индикаторов).
6. Category score (Расчёт по категориям, опционально).
7. Risk score (Итоговый риск).
...
*Примечание: Category Contribution is an interpretative decomposition and is not intended to reconstruct the global risk_score exactly. Сумма category_contribution не обязана строго равняться общему risk_score.*
...
Интервалы полуоткрытые справа (left-open, right-closed), кроме первого.
...
Демографическое расширение является ОПЦИОНАЛЬНЫМ (OUT OF SCOPE for v1.5 schema), таблицы состояния P1/P2/P3, Births, Deaths, Migration не представлены в DDL и должны сохраняться в JSON store или реализовываться как отдельный модуль.
