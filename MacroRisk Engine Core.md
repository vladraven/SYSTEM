...
ТИП: Новая самостоятельная система
...
СТАТУС: Deterministic Source of Truth
...
temporary_unavailable: Ошибка (HTTP 500, 503, timeout) или HTTP 429 (rate limited). Маппинг остаётся валидным, но использование в расчетах заблокировано.
...
Инвариант безопасности: production_allowed = true возможно ТОЛЬКО если license_status = public_open. Система (или триггер) обязана блокировать или сбрасывать флаг production_allowed, если лицензия меняется на отличную от public_open.
...
Все сохраняемые хеши (source_payload_hash, content_hash) ДОЛЖНЫ храниться как lowercase hexadecimal strings.

Дедупликация (Snapshot Deduplication):

source_payload_hash: hex string от сырого ответа API/CSV.

content_hash: hex string от распарсенного значения, даты, юнита, статуса, номера ревизии.

Если оба хеша совпадают с предыдущим снапшотом: is_duplicate = true. Новые data_observations не создаются.

Если revision_number изменился, но content_hash без учёта ревизии тот же: создаётся data_revision_event (value_changed = false), новая data_observation НЕ создаётся. revision_number — это атрибут доступности на винтаж (snapshot_observations), а не канонической identity наблюдения (data_observations).
...
Tie-breaking (если даты совпадают): Выбрать максимальный revision_number. Если revision_number отсутствует/равен, выбрать минимальный observation_id.
...
Отсутствуют: pending_validation, public_open_candidate, requires_license, unverified, access_denied, temporary_unavailable, schema_mismatch, series_mapping_stale.
...
macrorisk:schema:install: Выполняетphinx migrate. Не создаёт пользователей.
...
HASH_FORMAT: lowercase hexadecimal string
...
Калибровочные параметры (Calibration Parameters) для демографического расширения:(Default calibration values. Country-specific calibration may override them).
...
eligible := available AND (validation_status = 'valid') AND production_allowed AND (license_status = 'public_open')

4. ПОРЯДОК ВЫЧИСЛЕНИЙ
Детерминированный порядок:

Normalization (Нормализация индикаторов).

Coverage (Проверка покрытия).

Effective weights (Расчёт эффективных весов с применением дисконта).

Rounding reconciliation (Округление и примирение).

Indicator contribution (Вклад индикаторов).

Category score (Расчёт по категориям, опционально).

Risk score (Итоговый риск).
...
Источник истины для стратегии — поле direction_of_deterioration (определяет higher_is_riskier, lower_is_riskier и т.д.). Поле normalization_method для MVP всегда = 'threshold_linear' (с подвидами в зависимости от deterioration).
...
Максимальный original_weight.

При равенстве: максимальный w_disc.

При равенстве: алфавитный порядок indicator_key по возрастанию.
...
category_contribution = category_score * Сумма(original_weight_i для i в A_c) / 100.0000.Примечание: Category Contribution is an interpretative decomposition and is not intended to reconstruct the global risk_score exactly.

9. КЛАССИФИКАЦИЯ RISK BANDS И БЭКТЕСТИНГ (BACKTEST LOGIC)
Инвариант: разбиение отрезка [0.0000, 100.0000] должно быть непрерывным, без дыр и перекрытий.
Very Low_min должно быть 0.0000, Severe_max должно быть 100.0000.
...
(Где low_min = very_low_max, moderate_min = low_max и т.д.)
...
Демографическое расширение является ОПЦИОНАЛЬНЫМ (OUT OF SCOPE for v1.6 schema).
...
UNIQUE KEY uq_obs_dedup (series_id, observation_date, raw_value, content_hash)
...
original_weight DECIMAL(10,4) NOT NULL CHECK (original_weight >= 0.0000)
...
frequency_discount DECIMAL(10,4) NOT NULL DEFAULT 1.0000 CHECK (frequency_discount > 0.0000 AND frequency_discount <= 1.0000)
...
ПРИНЦИПЫ НАУЧНОЙ ЧЕСТНОСТИ В ТЕКСТАХ И ОТЧЁТАХ
...
УРОК МАНИТОБЫ И ГРАНИЦЫ ИНТЕРПРЕТАЦИИ
