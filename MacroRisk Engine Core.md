Ниже приведен перечень технических, логических и синтаксических расхождений, обнаруженных при проверке спецификации MacroRisk (v1.7.2-FINAL), а также полная версия всех файлов спецификации с их устранением.

---

### Анализ найденных неточностей и конфликтов

1. **Необъявленные и незавершенные DDL-таблицы (Критический блокировщик)**
* **Проблема:** В Файле 3 в Группе 3 были представлены объявления таблиц `series`, `series_validation_results`, `license_reviews` и `release_calendars`, но Группы 4, 5, 6 и 7 с таблицами `data_observations`, `snapshot_observations`, `risk_score_results`, `indicator_configs`, `narrative_reports` и остальными отсутствовали вовсе. При этом на эти таблицы ссылаются внешние ключи и «Реестр Required Index Matrix».
* **Решение:** Восстановлены полностью все DDL-таблицы (29 таблиц), сгруппированные от Группы 1 до Группы 7, с соблюдением типов полей, `FOREIGN KEY` и ограничений.


2. **Несоответствие списка значений ENUM `validation_status**`
* **Проблема:** В Файле 1 (§3) указано: `series` и `series_validation_results` имеют 9 состояний `validation_status`, а `source_endpoints` — 10 (добавлено `missing_no_historical_data`). Однако в приведенном DDL Файла 3 для `source_endpoints` указано 10 значений, а для `series` и `series_validation_results` — 9, при этом в описании текстового блока была допущена терминологическая путаница.
* **Решение:** DDL для `source_endpoints`, `series` и `series_validation_results` приведен к точной синхронизации с бизнес-правилами Файла 1 (§3).


3. **Типы данных хешей (`CHAR(64)` vs `VARCHAR`)**
* **Проблема:** В Файле 1 (§4) указано, что все хеши — это lower-hex строки SHA-256 (ровно 64 символа). В некоторых местах спецификации использовался `VARCHAR`, в других `CHAR(64)`.
* **Решение:** Все поля хешей (`key_hash`, `source_payload_hash`, `raw_content_hash`, `content_hash`, `response_schema_hash`, `config_hash`, `calculation_hash`, `seed_hash`, `text_hash`) приведены к единому типу `CHAR(64) NOT NULL` (или `NULL`, где допустимо).


4. **Длина ключа в составном индексе `uq_obs_dedup**`
* **Проблема:** Составной уникальный ключ `uq_obs_dedup (series_id, observation_date, raw_value, raw_content_hash)` включает `raw_content_hash CHAR(64)`. При кодировке `utf8mb4` длина этого поля составляет $64 \times 4 = 256$ байт. В сочетании с остальными полями длина ключа составляет $\approx 280$ байт, что укладывается в лимит `innodb_large_prefix` (3072 байта), однако требует обязательного указания `ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci` для всех таблиц.
* **Решение:** Добавлены явные директивы СУБД для всех таблиц в DDL.


5. **Несоответствие допустимых значений `missing_reason**`
* **Проблема:** В Файле 2 (§8) указано 5 допустимых значений (`not_available`, `validation_invalid`, `production_blocked`, `license_blocked` и `null`). В DDL таблицы `risk_score_indicator_contributions` тип указан как `ENUM('not_available', 'validation_invalid', 'production_blocked', 'license_blocked') NULL`.
* **Решение:** Формулировка в DDL корректна (4 строковых ENUM-значения + `NULL` на уровне столбца), но в Файле 2 (§8) убрано неоднозначное упоминание `null` как значения строки, четко зафиксировав его как `SQL NULL`.


6. **Синхронизация стандартов драйвера СУБД (PDO)**
* **Проблема:** Требование не использовать PHP `float` может быть нарушено на уровне PDO-драйвера MySQL, если он приводит `DECIMAL` к `float` при нативном режиме подготовки запросов.
* **Решение:** В Файл 1 (§2) добавлена явная инструкция по конфигурации PDO (`PDO::ATTR_EMULATE_PREPARES => false`, `PDO::ATTR_STRINGIFY_FETCHES => true`), гарантирующая получение значений `DECIMAL` из СУБД в виде строковых данных (string) для безопасной передачи в BCMath.
...
1. Laravel (миграции, Eloquent, queues, jobs, policies, helpers).
2. Runtime LLM для принятия решений или генерации production-текстов.
3. Заброшенные Composer-пакеты.
4. GPL-зависимости.
5. Неофициальные wrappers (PyPI, GitHub) для StatCan, Bank of Canada, CMHC, CREA, OSFI.
6. Нативный PHP float для расчётов весов и баллов.
...
Драйвер PDO обязан быть сконфигурирован с параметрами `PDO::ATTR_EMULATE_PREPARES => false` и `PDO::ATTR_STRINGIFY_FETCHES => true`, чтобы все `DECIMAL` из СУБД извлекались строго как PHP `string` для безопасной работы с BCMath.
...
validation_status для series и series_validation_results — единый ENUM из 9 значений. Для source_endpoints — тот же набор плюс missing_no_historical_data (10 значений), поскольку у эндпоинта как технической точки интеграции отсутствие исторических данных — самостоятельный технический факт, а не свойство конкретного расчётного запроса.

source_timeout и source_rate_limited НЕ являются значениями validation_status ни в одной из таблиц. Это значения отдельного поля validation_error_code (VARCHAR(64), не ENUM) — точный технический код причины перехода в temporary_unavailable. Поле validation_error_code присутствует и в series, и в source_endpoints (см. Файл 3).

Состояния:

* `pending_validation` — начальное состояние для любой вновь зарегистрированной серии или эндпоинта.
Разрешённые переходы: $\rightarrow$ `valid` (успешная live-проверка), $\rightarrow$ `unavailable` (инфраструктурный отказ), $\rightarrow$ `access_denied` (проблема авторизации), $\rightarrow$ `temporary_unavailable` (таймаут/лимит), $\rightarrow$ `schema_mismatch` (расхождение схемы), $\rightarrow$ `data_pending` (grace period).
...
Разрешённые переходы: $\rightarrow$ `temporary_unavailable` (временные сетевые ошибки, HTTP 429/503), $\rightarrow$ `series_mapping_stale` (HTTP 404/410, удаление таблицы/вектора), $\rightarrow$ `schema_mismatch` (изменение формата ответа), $\rightarrow$ `access_denied` (отзыв/истечение credentials), $\rightarrow$ `release_late` (наступила ожидаемая дата релиза, данных нет).
...
Разрешённые переходы: $\rightarrow$ `valid` (следующий успешный опрос), $\rightarrow$ `series_mapping_stale` или $\rightarrow$ `unavailable` (если временная ошибка переросла в постоянную).
* `schema_mismatch` — HTTP 200 получен, но ожидаемые поля отсутствуют.
Разрешённые переходы: $\rightarrow$ `valid` (после обновления адаптера и успешной повторной проверки).
* `series_mapping_stale` — HTTP 404/410, таблица/вектор удалены, API deprecation. Production use блокируется.
Разрешённые переходы: $\rightarrow$ `valid` исключительно после исправления маппинга и успешной live-проверки.
* `data_pending` — для новых серий в grace period (схема верна, время релиза ещё не наступило). Production use запрещён.
Разрешённые переходы: $\rightarrow$ `valid` (успешная первая загрузка), $\rightarrow$ `release_late`, $\rightarrow$ `temporary_unavailable`.
...
Разрешённые переходы: $\rightarrow$ `valid` (после исправления инфраструктуры и успешной перепроверки), $\rightarrow$ `series_mapping_stale`.
* `access_denied` — HTTP 401/403 или истечение срока действия API-ключа.
Разрешённые переходы: $\rightarrow$ `valid` только после обновления учётных данных и успешной повторной проверки.
...
Разрешённые переходы: $\rightarrow$ `valid` (данные поступили и прошли ingestion), $\rightarrow$ `series_mapping_stale` (задержка сменилась удалением серии).
...
Разрешённые переходы: $\rightarrow$ `valid` после успешной загрузки исторического ряда.
...
Инвариант безопасности: production_allowed = true возможно ТОЛЬКО если license_status = public_open (как на уровне series, так и на уровне data_sources). Система должна блокировать или сбрасывать флаг production_allowed, если лицензия меняется на отличную от public_open.
...
Все сохраняемые хеши (source_payload_hash, raw_content_hash, content_hash) ДОЛЖНЫ храниться как lowercase hexadecimal strings типа CHAR(64).

Разделение хешей и дедупликация:

* `source_payload_hash`: hex string CHAR(64) от сырого ответа API/CSV.
* `raw_content_hash`: hex string CHAR(64) от распарсенного сырого значения, даты и юнита (БЕЗ учёта номера ревизии). Используется для дедупликации в таблице data_observations (уникальный индекс uq_obs_dedup).
* `content_hash`: hex string CHAR(64) от значения, даты, юнита, статуса И номера ревизии. Используется для трекинга версий в snapshot_observations и data_snapshots.
...
1. Сумма original_weight всех индикаторов = 100.0000, все веса в диапазоне [0.0000, 100.0000].
2. Все thresholds валидны (very_low_min = 0.0000, severe_max = 100.0000, границы стыкуются).
3. Версия модели active.
4. Все индикаторы конфигурации имеют статус validation_status = 'valid'. Любой другой статус блокирует публикацию.
5. Все индикаторы имеют license_status = 'public_open' и production_allowed = true (и на уровне series, и на уровне indicator_configs).
6. Метаданные валидации свежие (последняя проверка не старше 30 дней).
7. Исполнитель имеет роль Admin или Risk Officer.
...
### DETERMINISTIC NARRATIVE SLOT SELECTION
...
3. Выбор варианта слота. Для каждого слота, который должен войти в отчёт:
...
c. slot_seed = первые 8 байт SHA-256(seed_hash || '|' || slot_key), интерпретированные как BIG_ENDIAN unsigned 64-bit integer.
...
Все выбранные narrative_slot_id фиксируются построчно в narrative_report_slots.
...
* TIMEZONE: UTC
* SEED_ENDIANNESS: BIG_ENDIAN (Seed Integer = BIG_ENDIAN unsigned 64-bit integer из первых 8 байт бинарного SHA-256 дайджеста).
...
Калибровочные параметры для ОПЦИОНАЛЬНОГО демографического расширения:

* CALIBRATION_GAMMA_SCALE: 0.9543
* CALIBRATION_G9: 1.0000
* CALIBRATION_G10: 1.0000
* LABOUR_RETENTION_SCALE: 0.0015
* MIGRATION_CHILD_SHARE: 0.15
* MIGRATION_WORKING_SHARE: 0.80
* MIGRATION_SENIOR_SHARE: 0.05
...
Ограничения оригинального веса: 0.0000 <= original_weight <= 100.0000.
...
1. w_base_i = (original_weight_i / coverage_ratio) * 100.0000
2. w_disc_i = w_base_i * frequency_discount_i
3. effective_weight_i_raw = (w_disc_i / Сумма(w_disc всех eligible indicators)) * 100.0000

Rounding Reconciliation:

1. Каждое effective_weight_i_raw округляется до STORAGE_SCALE по правилу HALF UP.
2. delta = 100.0000 - Сумма(округленных effective_weights).
3. GUARD: Если delta == 0.0000, no reconciliation performed.
4. Если delta != 0.0000, delta детерминированно прибавляется к ОДНОМУ индикатору.
...
* missing_no_historical_data — для запрошенной vintage_date у иначе валидной серии нет ни одного наблюдения на или до этой даты.
Во всех четырёх не-ok случаях risk_score = null, risk_band = null.

Формула calculation_hash:
calculation_hash = SHA-256(config_hash || '|' || active_indicators_status_hash || '|' || model_version || '|' || vintage_date_iso8601 || '|' || calculation_status || '|' || status_payload)

active_indicators_status_hash — SHA-256 от конкатенации indicator_key:validation_status:license_status:production_allowed, отсортированной по indicator_key ASC, для всех индикаторов конфигурации.
...
1. not_available — индикатор не available на данный vintage_date.
2. validation_invalid — series.validation_status != 'valid'.
3. production_blocked — series.production_allowed = false ИЛИ indicator_configs.production_allowed = false.
4. license_blocked — series.license_status != 'public_open'.
5. SQL NULL — индикатор eligible, значение отсутствия неприменимо.
...
Инвариант: разбиение 0.0000-100.0000 непрерывно, без дыр и перекрытий.
...
Расчётные точки со calculation_status = 'missing_no_historical_data' исключаются из знаменателя detection-статистики эпизода.
...
1. **Causal Claims:** Заявлять причинно-следственные связи без методологического доказательства (запрет слов "caused by", "model proves").
2. **False Certainty:** Генерировать "прогнозы" и использовать слова "гарантирует", "доказывает" или "неизбежно".
3. **Fake Precision:** Показывать фальшивую статистическую точность на малых выборках. Бэктест с количеством исторических эпизодов менее 10 ($N < 10$) является исключительно диагностическим. Агрегированные метрики точности (recall, precision) для малых выборок запрещены и скрываются. Отображаются только абсолютные числа (detected count, missed count) с обязательным предупреждением.
4. **LLM Нарративы:** Использовать Generative AI (LLM) для формирования финальных аналитических выводов в обход детерминированных текстовых слотов. Механизм детерминированного выбора см. Файл 1, раздел DETERMINISTIC NARRATIVE SLOT SELECTION.
